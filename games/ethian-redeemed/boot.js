...
function getBasePath() {
  ...
  // En local, utiliser un chemin relatif qui fonctionne avec Next.js
  return '/games/ethian-redeemed/';
}
...
function resolvePath(path) {
  // Si c'est déjà une URL absolue ou vide, la retourner telle quelle
  if (path.startsWith('http') || path === '') {
    return path;
  }
  
  // Si le chemin commence déjà par le chemin de base, ne pas le doubler
  if (path.startsWith(basePath)) {
    return path;
  }
  
  // Pour les chemins commençant par '/', déterminer s'il faut ajouter le basePath
  if (path.startsWith('/')) {
    // Si c'est un chemin absolu dans le domaine actuel, l'utiliser tel quel
    return path;
  }
